...
import { createSimulation } from "@/lib/simulations";
...
export async function saveGeneratedGroupRecords(generatedGroups, simulationInput = {}) {
  const supabase = createSupabaseServerClient();
  const simulation = await createSimulation({
    ...simulationInput,
    company: simulationInput.company || generatedGroups[0]?.[0]?.company || ""
  });
  const savedGroups = [];
  ...
  for (const [index, participants] of generatedGroups.entries()) {
    ...
    const { data: group, error: groupError } = await supabase
      .from(groupsTable)
      .insert({ company, groupName, simulationId: simulation.id })
      .select("*")
      .single();
    ...
    savedGroups.push({ ...group, simulation, participants });
  }
...
export async function getAdminGroupOverview() {
  const supabase = createSupabaseServerClient();
  const [groupsResult, membersResult, participantsResult, snapshotsResult, submissionsResult, simulationsResult] = await Promise.all([
    ...
    supabase.from(simulationsTable).select("*").order("createdAt", { ascending: false })
  ]);

  const error = groupsResult.error || membersResult.error || participantsResult.error || snapshotsResult.error || submissionsResult.error || simulationsResult.error;
  if (error) {
  ...
  const simulationsById = new Map((simulationsResult.data || []).map((simulation) => [simulation.id, simulation]));

  return (groupsResult.data || []).map((group) => ({
    ...group,
    simulation: simulationsById.get(group.simulationId) || null,
...
export async function updateGroupSimulation(groupId, simulationId) {
  const cleanGroupId = String(groupId || "").trim();
  const cleanSimulationId = String(simulationId || "").trim() || null;
  ...
  const { data, error } = await supabase
    .from(groupsTable)
    .update({ simulationId: cleanSimulationId })
    .eq("id", cleanGroupId)
    .select("*")
    .single();
